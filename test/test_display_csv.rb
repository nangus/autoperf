...
class TestAutoperfDisplayCSV < MiniTest::Unit::TestCase
  def setup
    @result = {
      100 => {
  ...
  def test_init_default
    display = Autoperf::Display::CSV.new(@result)
  ...
  def test_init_array
    ...
    display = Autoperf::Display::CSV.new(@result, table)
  ...
  def test_to_s
    display = Autoperf::Display::CSV.new(@result)
    result_string = "rate,connection_rate_per_sec,request_rate_per_sec,connection_time_avg,errors_total,reply_status_5xx,net_io_kb_sec\n100,10,10,10,2,2,1000\n"
  ...
  def test_print
    ...
    result_string = "rate,connection_rate_per_sec,request_rate_per_sec,connection_time_avg,errors_total,reply_status_5xx,net_io_kb_sec\n100,10,10,10,2,2,1000\n"
    assert_equal result_string, out
    assert_empty err
  end
end
